...
/* $Id: b57um.c,v 1.29.2.3 2009/11/25 06:28:45 Exp $ */
...
robo_fail:
	...
	*dev_out = dev;
	...
	pUmDevice->cih = ctf_attach(pUmDevice->osh, dev->name, &b57_msg_level);

	if (pUmDevice->cih == NULL) {
		B57_ERR(("ctf_attach() failed\n"));
		goto err_out_unmap;
	}

	ctf_dev_register(pUmDevice->cih, dev);
