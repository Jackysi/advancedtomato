...
static ssize_t usbdev_trig_name_show(struct class_device *dev,
				     char *buf)
{
...
static ssize_t usbdev_trig_interval_show(struct class_device *dev,
					 char *buf)
{
	struct led_classdev *led_cdev = class_get_devdata(dev);
	struct usbdev_trig_data *td = led_cdev->trigger_data;
...
static ssize_t usbdev_trig_interval_store(struct class_device *dev,
					  const char *buf,
					  size_t size)
{
	struct led_classdev *led_cdev = class_get_devdata(dev);
...
static CLASS_DEVICE_ATTR(activity_interval, 0644, usbdev_trig_interval_show,
...
static void usbdev_trig_activate(struct led_classdev *led_cdev)
{
	...
	rc = class_device_create_file(led_cdev->class_dev, &class_device_attr_activity_interval);
	if (rc)
...
static void usbdev_trig_deactivate(struct led_classdev *led_cdev)
{
	...
	if (td) {
		...
		class_device_remove_file(led_cdev->class_dev, &class_device_attr_device_name);
		class_device_remove_file(led_cdev->class_dev, &class_device_attr_activity_interval);
