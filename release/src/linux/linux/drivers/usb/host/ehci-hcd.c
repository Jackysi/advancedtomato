...

{
	...
	if (vendor_id == 0x31041106) {
		...
		printk(KERN_INFO "ECHI reg 0x49 = %x\n", misc_reg);
		misc_reg &= ~0x20;
		pci_write_config_byte(ehci->hcd.pdev, 0x49, misc_reg);
		pci_read_config_byte(ehci->hcd.pdev, 0x49, &misc_reg);
		printk(KERN_INFO "ECHI reg 0x49 changed to = %x\n", misc_reg);

		pci_read_config_byte(ehci->hcd.pdev, 0x4b, &misc_reg);
		printk(KERN_INFO "ECHI reg 0x4b = %x\n", misc_reg);
		misc_reg |= 0x20;
		pci_write_config_byte(ehci->hcd.pdev, 0x4b, misc_reg);
		pci_read_config_byte(ehci->hcd.pdev, 0x4b, &misc_reg);
		printk(KERN_INFO "ECHI reg 0x4b changed to = %x\n", misc_reg);
	}
}
...
dead:
		printk(KERN_ERR
		       "USB Error: A Host System error has been generated by "
		       "the EHCI interface.\n");
		printk(KERN_ERR
		       "USB Error: The EHCI USBSTS regiser is 0x%08lx.\n",
