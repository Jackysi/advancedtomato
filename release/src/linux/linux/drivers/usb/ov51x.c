...
/* 2.6 Doesn't support /proc/video, but this might still be defined */
 ...
 * Version Information
 */
#define DRIVER_VERSION "v1.65-1.12-mark"
#define EMAIL "mark@alpha.dyndns.org, joerg@heckenbach-aw.de"
#define DRIVER_AUTHOR "Mark McClelland <mark@alpha.dyndns.org> & Bret Wallach \
	& Orion Sky Lawlor <olawlor@acm.org> & Kevin Moore & Charl P. Botha \
	<cpbotha@ieee.org> & Claudio Matsuoka <claudio@conectiva.com> &\
	Joerg Heckenbach <joerg@heckenbach-aw.de>"
#define DRIVER_DESC "ov51x USB Camera Driver"
...
MODULE_PARM(jpeginfo, "i");
MODULE_PARM_DESC(jpeginfo, "Force information on JPEG size in first two bytes of frame buffer for OV519");
...
create_proc_ov511_cam(struct usb_ov511 *ov)
{
...
#endif
	PDEBUG(4, "creating /proc/video/ov51x/%s/", dirname);
	...
	PDEBUG(4, "creating /proc/video/ov51x/%s/info", dirname);
	...
	if (!snapshot) {
		...
		PDEBUG(4, "creating /proc/video/ov51x/%s/button", dirname);
	...
	PDEBUG(4, "creating /proc/video/ov51x/%s/control", dirname);
	lock_kernel();
...
destroy_proc_ov511_cam(struct usb_ov511 *ov)
{
	...
	if (ov->proc_control) {
		PDEBUG(4, "destroying /proc/video/ov51x/%s/control", dirname);
	...
	if (ov->proc_button) {
		PDEBUG(4, "destroying /proc/video/ov51x/%s/button", dirname);
	...
	/* Destroy "info" entry */
	if (ov->proc_info) {
		PDEBUG(4, "destroying /proc/video/ov51x/%s/info", dirname);
		remove_proc_entry("info", ov->proc_devdir);
		ov->proc_info = NULL;
	}

	/* Destroy per-device directory */
	PDEBUG(4, "destroying /proc/video/ov51x/%s/", dirname);
...
proc_ov511_create(void)
{
	...
	ov511_proc_entry = create_proc_entry("ov51x", S_IFDIR,
					     video_proc_entry);

	if (ov511_proc_entry)
		ov511_proc_entry->owner = THIS_MODULE;
	else
		err("Unable to create /proc/video/ov51x");
}

static void
proc_ov511_destroy(void)
{
	PDEBUG(3, "removing /proc/video/ov51x");

	if (ov511_proc_entry == NULL)
		return;

	remove_proc_entry("ov51x", video_proc_entry);
}
#else
