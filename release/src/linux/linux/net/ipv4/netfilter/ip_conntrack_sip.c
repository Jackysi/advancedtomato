...
/**** 2007-02-07 add by zg to fix cdrouter v3.4 item 321(cdrouter_sip_45) bug ****/
...
/**** 2007-02-07 end by zg to fix cdrouter v3.4 item 321(cdrouter_sip_45) bug ****/
		...
		enum ip_conntrack_info ctinfo)
{
	...
	DEBUGP("conntrack_sip: %u.%u.%u.%u:%u -> %u.%u.%u.%u:%u\n", 
        ...
        /**** 2007-02-07 add by zg to fix cdrouter v3.4 item 321(cdrouter_sip_45) bug ****/
	/**** Verify Via header port is used for response instead of UDP src port ****/
	if (dir == IP_CT_DIR_ORIGINAL)
	{                      
		const char *tmpdata=data;	              
		char *u=NULL;
		
		if ((u = strstr(tmpdata, "Via:")) != NULL)
	        {
			char tmpport[10];
			...
			tmpdata=u+4;
	                u= strstr(tmpdata, ":");
	                tmpdata=u+1;
			memcpy(tmpport,tmpdata,6);		 
		        if ((u= strstr(tmpport, ";"))!=NULL)
		        {
		        	*u = '\0';
		        }
		        if( (ntohs(ct->tuplehash[dir].tuple.src.u.udp.port))!= (u_int16_t)atoi(tmpport) &&
                            (ntohs(ct->tuplehash[dir].tuple.src.u.udp.port))!= SIP_PORT )
			{
                             	ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.udp.port = htons( (u_int16_t)atoi(tmpport) );					
			}
		}	
		else if ((u = strstr(tmpdata, "v:")) != NULL)
		{
			char tmpport[10];
			memset(tmpport,0,sizeof(tmpport));
			tmpdata=u+4;
...
static int __init init(void)
{
	...
	for (i = 0; (i < MAX_PORTS) && ports[i]; i++) {
		...
		/************** lzh add ***************************
		* when the sip packet destination port match 5056-5071,
		* not use src port to match, then come in sip alg
		* 2006.7.18
		**************************************************/	
#if 0
		...
		sip[i].mask.src.u.udp.port = 0xF0FF;
#endif
		//sip[i].tuple.dst.u.udp.port = htons(ports[i]);
