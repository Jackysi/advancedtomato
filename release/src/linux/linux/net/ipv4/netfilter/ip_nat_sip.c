		...
		struct sk_buff **pskb, u_int32_t newip)
{
	...
	if (strnicmp(data,"SIP/2.0",7) != 0) {
		natport = ntohs(ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.udp.port);
		...
		struct ip_conntrack_expect *expect)
{
	...
	if (expect) {
		...
		if (exp_sip_info->type == CONN_RTP) {
			...
			/* fullfill newtuple */
			/************ lzh start ****************
			* clean memory of this new tuple 
			* 2006.7.6
			***************************************/
		...
		struct ip_conntrack_expect *expect)
{
	...
	if (exp) {
		...
		/************* lzh add *******************************
		* we also need del timer when del this exp node
		* 2006.7.18
		*****************************************************/
		if (exp->expectant->helper->timeout && del_timer(&exp->timeout))
			ip_conntrack_unexpect_related(exp);
#ifdef RTCP_SUPPORT
		rtcpexp = expect_find(ct, wanip, port + 1);
		if (rtcpexp)
		{
			if (rtcpexp->expectant->helper->timeout && del_timer(&rtcpexp->timeout))
