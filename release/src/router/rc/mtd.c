 ...
 * FOR A SPECIFIC PURPOSE OR NONINFRINGEMENT CONCERNING THIS SOFTWARE.
 *
 * $Id: mtd.c,v 1.24 2005/05/16 02:29:50 honor Exp $
 */
...
mtd_erase(const char *mtd)
{
	...
	if(!strcmp(mtd, "nvram")) {
		if((check_action() == ACT_NVRAM_COMMIT))
			return 0;
		ACTION("ACT_ERASE_NVRAM");
	}
...
mtd_write(const char *path, const char *mtd)
{
	...
	for (erase_info.start = 0; erase_info.start < trx.len; erase_info.start += count) {
		...
		{
//			buf = buf + sizeof(struct trx_header);
//			count = count - sizeof(struct trx_header);
			lang_flag=100;

//			ioctl(mtd_fd, MEMERASE, &erase_info);
//			write(mtd_fd, buf+sizeof(struct trx_header), count-sizeof(struct trx_header));

			(void) ioctl(mtd_fd, MEMUNLOCK, &erase_info);
			if ( ioctl(mtd_fd, MEMERASE, &erase_info) != 0 || write(mtd_fd, buf+sizeof(struct trx_header), count-sizeof(struct trx_header)) != count-sizeof(struct trx_header)  ){
				perror(mtd);
				goto fail;
			}
