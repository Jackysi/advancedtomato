...
write_mac(const char *path)
{
	...
	ret = eval("misc", "-t", "set_mac", "-s", mac);

	printf("write_mac(): ret = %d\n", ret);
...
write_eou_key(const char *path)
{
	...
	snprintf(string, sizeof(string), "%s%s%s", device_id, private_key, public_key);

	eval("misc", "-t", "set_eou", "-s", string);
...
write_sn(const char *path)
{
	...
	eval("misc", "-t", "set_sn", "-s", buf);

	cprintf("Write sn done...\n");
