	...
	for (next = buf; (line = strsep(&next, "\n"));) {
		...
		if(!strcmp(type,"incoming")){
			if((!strncmp(in, "ppp", 3) && !strncmp(in, wan_if, 3)) ||
				...
				ret += websWrite(wp, "<TR bgcolor=cccccc> <TD align=middle height=1><SPAN STYLE=\"FONT-SIZE: 12pt\">%s%s</TD> <TD align=middle height=1><SPAN STYLE=\"FONT-SIZE: 12pt\">%s</TD></TR>\n",skip, src, servp ? servp->s_name : dpt);
			}
		}
			...
			if(!strncmp(in, lan_if, 3) &&
			   ...
			   (!strcmp(out, wan_if)))){
				...
				ret += websWrite(wp, "<TR bgcolor=cccccc> <TD align=middle><SPAN STYLE=\"FONT-SIZE: 12pt\">%s%s</TD><TD align=middle><SPAN STYLE=\"FONT-SIZE: 12pt\">%s</TD><TD align=middle><SPAN STYLE=\"FONT-SIZE: 12pt\">%s</TD></TR>\n",skip, src, dst, servp ? servp->s_name : dpt);	
			}
		}
		else if(!strcmp(type,"all")){
			int dir=0;
			if((!strncmp(out, "ppp", 3) && !strncmp(out, wan_if, 3)) ||      
