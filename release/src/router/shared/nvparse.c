 ...
 *
 * $Id: nvparse.c,v 1.19 2005/03/29 02:00:06 honor Exp $
 */
...
set_forward_port(int which, const netconf_nat_t *nat)
{
	char name[] = "forward_portXXXXXXXXXX", value[1000], *cur = value;
        char name1[] = "forward_portsipXXX", value1[100];
	int len;
	...
	snprintf(name, sizeof(name), "forward_port%d", which);
        snprintf(name1, sizeof(name1), "forward_portsip%d", which);
	len = sizeof(value);

        if(nat->match.src.ipaddr.s_addr)
	{
               snprintf(value1, sizeof(value1), inet_ntoa(nat->match.src.ipaddr));
		//cprintf("value1 = %s!!!\n", value1);
	}
	...
	/* Do it */
	//if (nvram_set(name, value))
        if (nvram_set(name, value) || nvram_set(name1, value1))
		return FALSE;
...
del_forward_port(int which)
{
	char name[] = "forward_portXXXXXXXXXX";
        char name1[] = "forward_portsipXXXXXXXXXX";

	snprintf(name, sizeof(name), "forward_port%d", which);
        snprintf(name1, sizeof(name1), "forward_portsip%d", which);
	
	ct_logger(LOG_INFO, "upnp[%d]: Del \"%s\" from \"%s\"", getpid(), nvram_safe_get(name), name);

	/* tanghui modify @ 2007-07-04 */
	/* keep the preset upnp forward entries */
	//return (nvram_unset(name) == 0) ? TRUE : FALSE;
#ifdef UPNP_FORWARD_SUPPORT
	if(which < 10)
	{
		return (nvram_unset(name1) == 0) ? TRUE : FALSE;
	}
	else
#endif
	{
		if(nvram_unset(name) == 0 && nvram_unset(name1) == 0)
			return TRUE;
		else
			return FALSE;
	}
