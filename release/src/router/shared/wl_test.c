...
char *
get_curpower(char *name)
{
	FILE *fp;
	char line[254];
	char string[254];
	static char value[254];
	...
	if((fp = fopen("/tmp/curpower", "r"))) {	// Get real value
		...
		while(fgets(line, sizeof(line), fp) != NULL) {
			...
			if(!strcmp(name, "cck") && !memcmp(string, patt1, strlen(patt1))) {
				cprintf("Get B phy CCK : [%s]\n", value);
				return value;
				
			}
			else if(!strcmp(name, "ofdm") && !memcmp(string, patt2, strlen(patt2))) {
				cprintf("Get B phy OFDM : [%s]\n", value);
				return value;
			}
		}
		fclose(fp);
	...
	switch(atoi(value)) {
		...
		case 1:
			...
			if((check_hw_type() == BCM4704_BCM5325F_CHIP)) {
				mysystem("wl txpwr1 -d -o 17");		// 2005-05-16, Set tx power to 17 dbm
			}
			else if((check_hw_type() == BCM4712_CHIP) || (check_hw_type() == BCM5325E_CHIP)) {
				mysystem("wl txpwr1 -d -o 16.5");	// 2005-03-04, Set tx power to 16.5 dbm
			}
			else if((check_hw_type() == BCM5352E_CHIP)) {
				mysystem("wl txpwr1 -d -o 18");		// 2005-05-16, Set tx power to 18 dbm
			}
			mysystem("wl curpower > /tmp/curpower");

			nvram_set("wl_cck_result", get_curpower("cck"));

			nvram_set("wl_ofdm_result", get_curpower("ofdm"));
...
/* For WRT54GV4.0 and WRT54GSv3.0, Added by honor 2005-05-25 */
int
