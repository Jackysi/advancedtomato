...
#endif
{
	...
	static struct option arg_options[] = {
		...
		{"lan_interface",	required_argument,	0, 'l'},
		{"now", 	no_argument,		0, 'n'},
	...
	for (;;) {
		...
		if (retval == 0) {
			...
			case BOUND:
				...
				/********************
				 * modify by tanghui @ 2006-03-22
				 * use original KERNEL MODE
				 ********************/
				//change_mode(LISTEN_KERNEL);
			...
			case REBINDING:
				...
				/*	Alpha add to send Gratuitous ARP when received DHCPACK 2007-02-13*/
					int ifindex;
					u_int32_t wan_ip;
					unsigned char wan_mac[6];
					/* Check network */
					if (client_config.interface && read_interface(client_config.interface, &ifindex, &wan_ip, wan_mac) >= 0)
					{
							int times = 3;
again:
							if(arpping(requested_ip, wan_ip, wan_mac, client_config.interface) == 0)
							{
								//cprintf("The offered IP address is exist in network. Skip!\n");
								//syslog(LOG_DEBUG, "(*DHCP-Client*)IP discarded, Offered IP address exists in the network.");
								xid = random_xid();
								send_decline(xid, requested_ip);
								requested_ip = 0;
								state = INIT_SELECTING;
								LOG(LOG_INFO, "IP conflict, entering init state");
								run_script(NULL, "deconfig");
								timeout = now + 10;
								change_mode(LISTEN_RAW);
								continue;
