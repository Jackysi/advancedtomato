...
{	
	...
	while(1) { /* loop until universe collapses */
		...
		switch (state[0]) {
		case DHCPDISCOVER:
			LOG(LOG_INFO,"received DISCOVER from %s", ether_etoa(packet.chaddr, mac));	// modify by honor
			...
			break;			
 		case DHCPREQUEST:
			LOG(LOG_INFO, "received REQUEST from %s", ether_etoa(packet.chaddr, mac));	// modify by honor
		...
		case DHCPDECLINE:
			...
			break;
		case DHCPRELEASE:
			LOG(LOG_INFO,"received RELEASE from %s", ether_etoa(packet.chaddr, mac));	// modify by honor
			cprintf("received RELEASE from %s\n", ether_etoa(packet.chaddr, mac));	// modify by honor
			if (lease) lease->expires = get_time(0);
			break;
		case DHCPINFORM:
			LOG(LOG_INFO,"received INFORM from %s", ether_etoa(packet.chaddr, mac));	// modify by honor
