...
/*
	...
	Notes:
	 * IGD 1.0 defines Layer3Forwarding as optional.
	 * This code doesn't do anything useful afaict.
	 * Still, might be expected by some. Upnptest, at least, needs to see this.
...
ServiceTemplate Template_Layer3Forwarding = {
    ...
    Actions,
	0,
	"urn:upnp-org:serviceId:L3Forwarding",
	NULL
};
...
int Layer3_GetVar(struct Service *psvc, int varindex)
{
    ...
    case VAR_DefaultConnectionService:
		if (data->default_svc != NULL) {
		    sprintf(var->value, "urn:schemas-upnp-org:service:%s:1,%s",
				data->default_svc->template->name,
...
static PService find_svc_by_type(PDevice pdev, char *type)
{
	...
	if (name) psvc = find_svc_by_name(pdev, name);
    return psvc;
}
