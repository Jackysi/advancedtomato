...
void foo_send(UFILE *upkt, struct sockaddr *dst, int dstlen)
{
#if 0
	int rsd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	if (rsd < 0) {
		perror("socket");
		return;
	}
	...
	osl_ifaddr("vlan1", &sai.sin_addr);
	sai.sin_family = AF_INET;
	sai.sin_port = 0;
	bind(rsd, (struct sockaddr *)&sai, sizeof(sai));

	sendto(rsd, ubuffer(upkt), utell(upkt), 0, dst, dstlen);
    ...
    if ((upkt = usopen(NULL, 0))) {
		forall_devices(pdev) {
		    if (strcmp(pdev->template->type, st) == 0) {
				...
				// build the SSDP packet
				ssdp_packet(upkt, SSDP_REPLY, pdev->template->type, tmpbuf, location(pdev, pif), max_age);
				
				foo_send(upkt, addr, addrlen);
		    ...
		    forall_devices(pdev) {
				...
				forall_services(pdev, psvc) {
					...
					if (strlen(psvc->template->name) == namelen && strncmp(psvc->template->name, name, namelen) == 0) {
						...
						// build the SSDP packet
						ssdp_packet(upkt, SSDP_REPLY, st, tmpbuf, loc, max_age);
...
static void advertise_device(PDevice pdev, ssdp_t sstype, struct iface *pif, struct sockaddr *addr, int addrlen)
{
	...
	if (ISROOT(pdev)) {
	    snprintf(tmpbuf, sizeof(tmpbuf), "%s::upnp:rootdevice", pdev->udn);
	    ssdp_packet(upkt, sstype, "upnp:rootdevice", tmpbuf, loc, max_age);
	    if (sendto(pif->ssdp_connection->fd, ubuffer(upkt), utell(upkt), 0, addr, addrlen) < 0)
		UPNP_ERROR(("sendto failed at %s:%d - err %d\n", __FILE__, __LINE__, errno));
	    uflush(upkt); 	// reset the packet buffer before we build another.
	}
	...
	ssdp_packet(upkt, sstype, pdev->udn, pdev->udn, loc, max_age);
	if (sendto(pif->ssdp_connection->fd, ubuffer(upkt), utell(upkt), 0, addr, addrlen) < 0)
	    UPNP_ERROR(("sendto failed at %s:%d - err %d\n", __FILE__, __LINE__, errno));
	uflush(upkt); 	// reset the packet buffer before we build another.
    ...
    const char *location, int Duration)
{
    ...
    case SSDP_REPLY:
		...
		uprintf(up,
			...
			"DATE: %s\r\n"
			"\r\n",
			location, SERVER, Usn, Duration, ntst, date);
		return;
/*
HTTP/1.1 200 OK
LOCATION: http://192.168.5.1:62550/
EXT:
SERVER: POSIX, UPnP/1.0, Intel MicroStack/1.0.1868
USN: uuid:c89c92db-b8d5-46e8-938f-f29bb20ab459::upnp:rootdevice
...
*/
    case SSDP_ALIVE:
		uprintf(up,
		    ...
		    "USN: %s\r\n"
			"\r\n",
			Duration, SERVER, SSDP_IP, SSDP_PORT, location, ntst, Usn);
			return;
/*
