...
<title>[<% ident(); %>] IP Traffic: History</title>
...
function redraw() {
	...
	if (daily_history.length > 0) {
		...
		for (i = 0; i < daily_history.length; ++i) {
			...
			h = h + '<a href="javascript:viewIptDetail(' + i + ')" title="View IP Traffic Details">[IPTraffic]</a>';
			h = h + '</small>';
			var ymd = getYMD(b[0]);
...
<div class='section-title'>IP Traffic History</div>
<div class='section'>
...
c.push({ title: 'Show stats by subnet', name: 'f_subnet', type: 'checkbox', suffix: ' <small>(Not considered when calculating total traffic on last the line)</small>' });
c.push({ title: 'Show hostnames', name: 'f_hostnames', type: 'checkbox' });
c.push({ title: 'Start Date', name: 'f_begin_date', type: 'select', options: [['0', 'Any']] });
c.push({ title: 'End Date', name: 'f_end_date', type: 'select', options: [['0', 'Any']] });
