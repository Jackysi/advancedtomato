...
<title>[<% ident(); %>] <% translate("Status"); %>: <% translate("Logs"); %></title>
...
<div id='logging'>
	<div class='section-title'><% translate("Logs"); %></div>
	<div class='section'>
		...
		<a href="logs/syslog.txt?_http_id=<% nv(http_id) %>"><% translate("Download Log File"); %></a><br><br>
		<input type="text" maxsize=32 size=33 id="find-text"> <input type="button" value="<% translate("Find"); %>" onclick="find()"><br>
		<br><br>
...
<script type='text/javascript'>
if (nvram.log_file != '1') {
	W('<div class="note-disabled"><% translate("Internal logging disabled"); %>.</b><br><br><a href="admin-log.asp"><% translate("Enable"); %> &raquo;</a></div>');
