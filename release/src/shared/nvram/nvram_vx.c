 ...
 *
 * $Id: nvram_vx.c,v 1.5.18.1 2005/09/23 06:27:05 honor Exp $
 */
		...
		*isemb = FALSE;
		...
		while (off <= lim) {
			...
			if (nvh->magic == NVRAM_MAGIC)
				{
				/* now check data integrity, checksum entire NVRAM */ 
				tmp.crc_ver_init = htol32(nvh->crc_ver_init);
				...
				crc = hndcrc8((char *) &nvh[1], nvh->len - sizeof(struct nvram_header), crc);
				/* now compare calculated checksum with one stored in NVRAM */
				if (crc == (nvh->crc_ver_init & 0xff))				
					return (nvh);	/* return if crc's match */
				}
			off <<= 1;
		};
	}
