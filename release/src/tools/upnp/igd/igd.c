...
/*Zhijian 2007-07-19 add for UPnP device name*/
static char router_name[128];
int IGDevice_Init(PDevice igdev, device_state_t state, va_list ap)
{
	...
	/*Zhijian 2007-07-19 add for UPnP device name*/
#if OEM == LINKSYS
	sprintf(router_name, "Linksys %s", nvram_safe_get("router_name"));
	
#else
	strcpy(router_name, nvram_safe_get("router_name"));
#endif
	igdev->friendlyname = router_name;
