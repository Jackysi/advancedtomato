...
static bool update_external_address(PService psvc)
{
    ...
    if (osl_wan_isup(pdevdata->ifname)) {
        /******** add by zg 2006.11.13 for cdrouter v3.3 upnp module bugs in pptp and pppoe mode ********/
        strcpy(pdevdata->ifname, nvram_safe_get("wan_iface"));
...
static int ForceTermination(UFILE *uclient, PService psvc, PAction ac, pvar_entry_t args, int nargs)
{
	...
	igd_restart(1);
#else
	nvram_set("action_service","upnp_teminate");
	kill(1, SIGUSR1);
#endif
    }
...
static int RequestConnection(UFILE *uclient, PService psvc, PAction ac, pvar_entry_t args, int nargs)
{
	...
	igd_restart(1);
#else
	char *ConnType = nvram_safe_get(wanproto);
	if(!strcmp(ConnType, "dhcp"))
	{
		nvram_set("action_service", "upnp_dhcp_renew");
	}
	...
	{
		nvram_set("action_service", "start_pptp");
	}
	else if (!strcmp(ConnType, "l2tp"))
	{
		nvram_set("action_service", "start_l2tp");
	}
	else if (!strcmp(ConnType, "heartbeat"))
	{
		nvram_set("action_service", "start_heartbeat");
	}
	kill(1, SIGUSR1);
#endif
	pdata->connection_status = IP_CONNECTING;
