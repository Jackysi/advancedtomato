...
void soap_action(UFILE *up, PService psvc, char *soapaction, char *body)
{
    ...
    char *sp, *eb;
    char pattern[80];
	memset(body_local,'\0',sizeof(body_local));	
	{
		int c=0;
		if(body!=NULL)
		{
			c = strlen(body);
			memcpy(body_local,body,c);	
		}
	}
...

	{
		...
		if((action = strstr(ac,"AddPortMapping" )) != NULL)             
		{                 
			for(i=0;i<nargs;i++)                 
			{                    
				if(strncmp(args[i].name, "NewInternalClient",strlen("NewInternalClient")) == 0)                     
				{                         
					union  ADDRESS
					{
						unsigned int v;
					...
					union  ADDRESS IP_ADDRESS;
					union  ADDRESS NET_MASK;
					...
					sscanf(args[i].value, "%d.%d.%d.%d", &IP_ADDRESS.b[0], &IP_ADDRESS.b[1], &IP_ADDRESS.b[2], &IP_ADDRESS.b[3]);
					sscanf(lan_netmask, "%d.%d.%d.%d", &NET_MASK.b[0], &NET_MASK.b[1], &NET_MASK.b[2], &NET_MASK.b[3]);
					
					compare = ((IP_ADDRESS.v) & (NET_MASK.v));
					if((compare==IP_ADDRESS.v)||(strcmp(args[i].value, LanIp) == 0))
					{
						soap_error( up, SOAP_INVALID_ARGS );                        
						//printf("soap error return 111\n");
						return ;
					}
					if((0xffffffff-NET_MASK.v)==((IP_ADDRESS.v)-compare))
					{                             
        					printf("find broadcast\n");
						{
							soap_error( up, SOAP_INVALID_ARGS );
							return ;
        					}    
